...
From a usability perspective, make sure to include a description and title that
clearly explains what the user is requesting and what will happen once they
submit their issue.

To ensure smooth operation, make sure to set the `label` and `projects`
properties so that your workflows are properly triggered.
...
and, depending on their values, may be processed differently. The
[`issue-ops/parser`](https://github.com/issue-ops/parser) action provides
specific output formats based on the `type` property of the input.
