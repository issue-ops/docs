...
jobs:
  team-request:
    ...
    outputs:
      request: ${{ steps.validate.outputs.request }}
    ...
    steps:
      - name: Parse Issue Body
        ...
        with:
          script: |
            const parse = require('./src/new-repo-request/parse.js')
            await parse(github, context, core)
      ...
      - if: ${{ steps.parse.outputs.success == 'true' }}
        ...
        with:
          ...
          script: |
            const validate = require('./src/new-repo-request/validate.js')
            const request = JSON.parse('${{ steps.parse.outputs.request }}')
            await validate(github, context, core, request)
    ...
    steps:
      - name: Add User to Team
        ...
        with:
          github-token: ${{ steps.token.outputs.token }}
          script: |
            // Get the request data from the `team-request` job
