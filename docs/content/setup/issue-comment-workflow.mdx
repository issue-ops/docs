...
After the issue has been created and any initial processing has been run, the
issue comment workflow becomes the main driver of the rest of the process. This
...
The issue comment workflow should, at minimum, be triggered by creation of new
...
general, any workflow that involves processing issue comments should start by
looking for the specific command being run.
...
Most issue comment workflows can be broken down into the following steps.
...
By default, any user with read access to a repository can create issues and add
...
automatically add a reaction to any issue comments that contain a command.
