...
`Note` is a [shortcode](https://mdxjs.com/blog/shortcodes), meaning it's
globally available in all `.md` and `.mdx` files. So you can use the `Note`
...
To write markdown syntax inside of a `Note`, make sure to add extra newlines to
separate the content from the opening and closing tags:
...
Use the `danger` variant to indicate that something bad could happen if the
reader ignores the note.
