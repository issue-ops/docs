...
<Note variant="warning">
  Anyone with access to create issues can also change labels! Labels are good
  for state tracking, but should not be used to determine if a request is valid!
  For more information, see the [Validate](/workflow/validate) step.
</Note>
...
add issues as they are created, and use lifecycle rules to keep track of the
...
For more information, see [GitHub Apps](/reference/github-apps).
