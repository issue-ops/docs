...
This will be the start of the IssueOps workflow. In order to do anything with
the request, we need to parse the issue body and extract the information we need
to get approval and create a repository.

<Note>Check the workflow comments for useful tips and tricks!</Note>
...
jobs:
  new-repo-request:
    ...
    if: contains(github.event.issue.labels.*.name, 'issueops:new-repo-request')
