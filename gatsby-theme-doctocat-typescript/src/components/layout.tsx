...
const getPageAncestry = (url: string, object: any) => {
  ...
  const buildArray = (node: any, path: string) => {
    if (node.url === path) {
...
export default function Layout({
  ...
  const breadcrumbData = getPageAncestry(path, navItems).filter(
    (item) => item.url
  )
        ...
        <Box
          ...
          {pageContext.tableOfContents.items ? (
            <Box
              ...
              style={{ gridArea: 'table-of-contents', overflow: 'auto' }}>
              <Heading
                as="h3"
                sx={{
                  fontSize: 1,
          ...
          <Box sx={{ width: '100%', maxWidth: '960px' }}>
            <Box as="main" id="skip-nav" sx={{ mb: 4 }}>
              ...
              {description ? (
                <Box sx={{ fontSize: 3, mb: 3 }}>{description}</Box>
              ) : null}
              {status ||
              source ||
              storybook ||
